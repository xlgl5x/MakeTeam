...
async def make(ctx, mix_type):
    ...
    if mix_type == 'mix1':
        voice_channel = discord.utils.get(ctx.guild.voice_channels, name='[MIX 1] - LOBBY')
        team_1_channel = discord.utils.get(ctx.guild.voice_channels, name='[MIX 1] - EQUIPE 1')
        team_2_channel = discord.utils.get(ctx.guild.voice_channels, name='[MIX 1] - EQUIPE 2')
    elif mix_type == 'mix2':
        ...
        await ctx.send('Comando inválido. Use !comandos para ver a lista de comandos disponíveis.')
        await comandos(ctx)
        return
    ...
    if len(members) < 2:
        await ctx.send(f'O canal "{mix_type.upper()} - LOBBY" precisa ter pelo menos 2 membros (excluindo bots) para formar equipes.')
        return
    ...
    if not team_1_channel or not team_2_channel:
        await ctx.send(f'Canal de voz "[MIX 1] - EQUIPE 1" ou "[MIX 1] - EQUIPE 2" não encontrado')
        return
    ...
    if queue:
        queue_channel = discord.utils.get(ctx.guild.voice_channels, name='FILA DE ESPERA')
...
async def move(ctx, mix_type):
    if mix_type == 'mix1':
        team_1_channel = discord.utils.get(ctx.guild.voice_channels, name='[MIX 1] - EQUIPE 1')
        team_2_channel = discord.utils.get(ctx.guild.voice_channels, name='[MIX 1] - EQUIPE 2')
        mix_channel = discord.utils.get(ctx.guild.voice_channels, name='[MIX 1] - LOBBY')
    elif mix_type == 'mix2':
        team_1_channel = discord.utils.get(ctx.guild.voice_channels, name='[MIX 2] - EQUIPE 1')
        team_2_channel = discord.utils.get(ctx.guild.voice_channels, name='[MIX 2] - EQUIPE 2')
        mix_channel = discord.utils.get(ctx.guild.voice_channels, name='[MIX 2] - LOBBY')
    else:
        await ctx.send('Comando inválido. Use !comandos para ver a lista de comandos disponíveis.')
        await comandos(ctx)
        return
    ...
    await ctx.send(f'Todos os membros das equipes foram movidos para o canal "{mix_channel}".')
